...
businessRouter.get('/auth/me', authRequired, async (req: AuthedRequest, res) => {
  const { rows } = await query(`SELECT id, name, email, slug, reward_name, reward_threshold, theme_color
                                FROM businesses WHERE id=$1`, [req.user!.businessId]);
...
businessRouter.post('/program', authRequired, async (req: AuthedRequest, res) => {
  ...
  const { rows } = await query(`UPDATE businesses
