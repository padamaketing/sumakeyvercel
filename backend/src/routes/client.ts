...
clientRouter.post('/client/register', async (req, res) => {
  ...
  if (!clientId) {
    const ins = await query(`INSERT INTO clients (name, email, phone) VALUES ($1,$2,$3) RETURNING id`, [name, email || null, phone || null]);
    clientId = ins.rows[0].id;
  }

  await query(`
    INSERT INTO memberships (business_id, client_id) VALUES ($1,$2)
...
clientRouter.get('/client/:id', authRequired, async (req: AuthedRequest, res) => {
  const { id } = req.params;
  const { rows } = await query(`
    SELECT c.id, c.name, c.email, c.phone, m.stamps, m.rewards_pending, m.last_scan_at
    FROM memberships m
